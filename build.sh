...
mvn install
cd target 
unzip ets-wms-sld11-1.1.0-r2.zip -d $TE_BASE/scripts/
...
echo "  "
echo "TEAM Engine ($te_tag) and the ets-wms-sld11-1.1.0-r2 have been installed."
echo "  "
echo " UNIX/OS users - You can run the test with the following commands: "
echo "   export TE_BASE=$TE_BASE"
echo "   $base/teamengine/te_install/bin/unix/test.sh -source=wms-sld/1.1.0/ctl/wms-sld.xml"
echo "  "
echo " Windows users - You can run the test with the following commands:"
echo "   set TE_BASE=$TE_BASE"
echo "   $base/teamengine/te_install/bin/windows/test.bat -source=wms-sld/1.1.0/ctl/wms-sld.xml"
echo "  "
